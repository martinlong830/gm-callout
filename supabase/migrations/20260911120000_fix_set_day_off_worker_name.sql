-- Fix: set_day_off must honor payload worker_name (including clear → null).
-- Prior coalesce(c.worker_name, excluded.worker_name) kept stale names and
-- blocked Person assigns on all-day-off rows (Eugene → Unassigned).
    ...
    if v_op_type = 'add_slot' then
      ...
      on conflict (company_id, restaurant_id, day_iso, role, slot_key) do update set
        ...
        worker_name = case
          ...
          when v_op_type = 'set_day_off' and (v_payload ? 'worker_name') then excluded.worker_name
          when v_op_type = 'set_day_off' then c.worker_name
