-- Per-punch paid vs unpaid break override (null = inherit shift / employee default).

alter table public.time_clock_entries
  add column if not exists break_paid boolean;

comment on column public.time_clock_entries.break_paid is
  'When true, break minutes count as paid time; when false, deducted; null inherits shift/employee policy.';
...
grant execute on function public.manager_save_time_clock_entry(
  uuid, uuid, timestamptz, timestamptz, integer, text, jsonb, timestamptz, timestamptz, boolean
) to authenticated;
