-- Manager timecard save: persist break start/end timestamps (same as kiosk).
...
begin
  ...
  select exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'time_clock_entries' and column_name = 'break_start_at'
  ) into has_break_times;
  select exists (
    select 1 from information_schema.columns
  ...
  if target_id is not null then
    if has_break and has_shift_id and has_history and has_break_times then
      ...
      where id = target_id;
    elsif has_break and has_shift_id and has_history then
      update public.time_clock_entries
      set
        clock_in_at = p_clock_in_at,
        clock_out_at = p_clock_out_at,
  ...
  if target_id is not null then
    ...
    if has_break and has_shift_id and has_history and has_break_times then
      insert into public.time_clock_entries (
        employee_id, clock_in_at, clock_out_at, break_minutes,
        break_start_at, break_end_at, schedule_shift_id, edit_history
      )
      values (
        p_employee_id,
        p_clock_in_at,
