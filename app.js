(function () {
  ...
  /**
   * Manual group-order / potential grid: mondayIso → restaurantId → platformId → dayIso → text.
   * Synced inside draft_schedule (same cloud path as slot order).
   */
  ...
  function sanitizeGroupOrderPotentialByWeek(raw) {
    if (!raw || typeof raw !== 'object') return {};
    var out = {};
    var platformIds = GROUP_ORDER_POTENTIAL_PLATFORMS.map(function (p) {
      return p.id;
    });
      ...
      Object.keys(byRest).forEach(function (rid) {
        ...
        platformIds.forEach(function (pid) {
          ...
          Object.keys(days).forEach(function (dayKey) {
            var iso = String(dayKey || '').slice(0, 10);
  ...
  function mergeGroupOrderPotentialByWeekMaps(localMap, remoteMap, preferWhenBoth) {
    preferWhenBoth = preferWhenBoth === 'local' ? 'local' : 'remote';
    var local = sanitizeGroupOrderPotentialByWeek(localMap);
    var remote = sanitizeGroupOrderPotentialByWeek(remoteMap);
    var out = {};
    var weekKeys = {};
    ...
    Object.keys(weekKeys).forEach(function (mon) {
      ...
      Object.keys(restKeys).forEach(function (rid) {
        ...
        var rPlat = rRest[rid] || {};
        var platKeys = {};
        Object.keys(lPlat).forEach(function (k) {
          platKeys[k] = true;
        });
        Object.keys(rPlat).forEach(function (k) {
          platKeys[k] = true;
        });
        var platOut = {};
        Object.keys(platKeys).forEach(function (pid) {
          var lDays = lPlat[pid] || {};
          var rDays = rPlat[pid] || {};
          var dayKeys = {};
          ...
          var dayOut = {};
          Object.keys(dayKeys).forEach(function (iso) {
            var lv = lDays[iso] != null ? String(lDays[iso]).trim() : '';
            var rv = rDays[iso] != null ? String(rDays[iso]).trim() : '';
  ...
  function getGroupOrderPotentialCell(restaurantId, mondayIso, platformId, dayIso) {
    var mon = normalizeSlotOrderMondayIso(mondayIso);
    var rid = restaurantId || currentRestaurantId;
    var iso = String(dayIso || '').slice(0, 10);
    if (!mon || !rid || !platformId || !iso) return '';
    var week = groupOrderPotentialByWeekStore[mon];
    var rest = week && week[rid];
    var plat = rest && rest[platformId];
    var val = plat && plat[iso];
    return val != null ? String(val) : '';
  }

  /** Empty cells display as 0 in the schedule grid (storage stays sparse until edited). */
  function displayGroupOrderPotentialCell(restaurantId, mondayIso, platformId, dayIso) {
    var val = getGroupOrderPotentialCell(restaurantId, mondayIso, platformId, dayIso);
    return val !== '' ? val : '0';
  }

  function setGroupOrderPotentialCell(restaurantId, mondayIso, platformId, dayIso, value) {
    var mon = normalizeSlotOrderMondayIso(mondayIso);
    var rid = restaurantId || currentRestaurantId;
    var iso = String(dayIso || '').slice(0, 10);
    if (!mon || !rid || !platformId || !iso) return;
    if (!groupOrderPotentialByWeekStore[mon]) groupOrderPotentialByWeekStore[mon] = {};
    if (!groupOrderPotentialByWeekStore[mon][rid]) groupOrderPotentialByWeekStore[mon][rid] = {};
    if (!groupOrderPotentialByWeekStore[mon][rid][platformId]) {
      groupOrderPotentialByWeekStore[mon][rid][platformId] = {};
    }
    var s = value != null ? String(value).trim().slice(0, 80) : '';
    if (!s) {
      delete groupOrderPotentialByWeekStore[mon][rid][platformId][iso];
      if (!Object.keys(groupOrderPotentialByWeekStore[mon][rid][platformId]).length) {
        delete groupOrderPotentialByWeekStore[mon][rid][platformId];
      }
      if (!Object.keys(groupOrderPotentialByWeekStore[mon][rid]).length) {
        delete groupOrderPotentialByWeekStore[mon][rid];
      }
      if (!Object.keys(groupOrderPotentialByWeekStore[mon]).length) {
        delete groupOrderPotentialByWeekStore[mon];
      }
    } else {
      groupOrderPotentialByWeekStore[mon][rid][platformId][iso] = s;
    }
    groupOrderPotentialByWeekStore = sanitizeGroupOrderPotentialByWeek(groupOrderPotentialByWeekStore);
    persistGroupOrderPotentialStore();
  }

  function loadGroupOrderPotentialStore() {
    try {
      var raw = localStorage.getItem(GROUP_ORDER_POTENTIAL_KEY);
      if (!raw) return {};
      return sanitizeGroupOrderPotentialByWeek(JSON.parse(raw));
    } catch (_e) {
      return {};
    }
  }
  ...
  function persistGroupOrderPotentialStore(opts) {
    try {
      localStorage.setItem(
        GROUP_ORDER_POTENTIAL_KEY,
        JSON.stringify(sanitizeGroupOrderPotentialByWeek(groupOrderPotentialByWeekStore || {}))
      );
  ...
  /**
   * Before pushing a dirty local draft over remote, absorb remote group-order cells the
   * local client has not edited since last confirmed push (same idea as slot-order absorb).
   */
  function absorbUnchangedRemoteGroupOrderFromDraft(remoteDr) {
    ...
    try {
      ...
      if (confirmedRaw) {
        ...
        confirmedGroup = sanitizeGroupOrderPotentialByWeek(
          confirmedObj && confirmedObj.groupOrderPotentialByWeek
        );
      }
    } catch (_c) {
      confirmedGroup = {};
    }
    var remoteGroup = sanitizeGroupOrderPotentialByWeek(remotePayload.groupOrderPotentialByWeek);
    var localGroup = sanitizeGroupOrderPotentialByWeek(groupOrderPotentialByWeekStore);
    var localJson = JSON.stringify(localGroup);
    var confirmedJson = JSON.stringify(confirmedGroup);
    if (localJson !== confirmedJson) {
      /* Local has unpushed group-order edits — merge remote into local without clobbering. */
      var mergedDirty = mergeGroupOrderPotentialByWeekMaps(localGroup, remoteGroup, 'local');
      if (JSON.stringify(mergedDirty) !== localJson) {
        groupOrderPotentialByWeekStore = mergedDirty;
        persistGroupOrderPotentialStore({ skipDirty: true });
      }
      return;
    }
    var remoteJson = JSON.stringify(remoteGroup);
    if (localJson === remoteJson) return;
    groupOrderPotentialByWeekStore = remoteGroup;
    persistGroupOrderPotentialStore({ skipDirty: true });
  }

  function sanitizeScheduleNetSalesByWeek(raw) {
    if (!raw || typeof raw !== 'object') return {};
    var out = {};
    Object.keys(raw).forEach(function (weekKey) {
      var mon = normalizeSlotOrderMondayIso(weekKey);
  ...
  function mergeScheduleNetSalesByWeekMaps(localMap, remoteMap, preferWhenBoth) {
    preferWhenBoth = preferWhenBoth === 'local' ? 'local' : 'remote';
    var local = sanitizeScheduleNetSalesByWeek(localMap);
    var remote = sanitizeScheduleNetSalesByWeek(remoteMap);
    var out = {};
    var weekKeys = {};
    Object.keys(local).forEach(function (k) {
      weekKeys[k] = true;
  ...
  function getScheduleNetSalesCell(restaurantId, mondayIso, dayIso) {
    var mon = normalizeSlotOrderMondayIso(mondayIso);
    var rid = restaurantId || currentRestaurantId;
    var iso = String(dayIso || '').slice(0, 10);
    if (!mon || !rid || !iso) return '';
    var week = scheduleNetSalesByWeekStore[mon];
    var rest = week && week[rid];
    var val = rest && rest[iso];
    return val != null ? String(val) : '';
  }

  function setScheduleNetSalesCell(restaurantId, mondayIso, dayIso, value) {
    var mon = normalizeSlotOrderMondayIso(mondayIso);
    var rid = restaurantId || currentRestaurantId;
    var iso = String(dayIso || '').slice(0, 10);
    if (!mon || !rid || !iso) return;
    if (!scheduleNetSalesByWeekStore[mon]) scheduleNetSalesByWeekStore[mon] = {};
    if (!scheduleNetSalesByWeekStore[mon][rid]) scheduleNetSalesByWeekStore[mon][rid] = {};
    var cleaned = value != null ? String(value).trim().replace(/[$,\s]/g, '') : '';
    var n = cleaned === '' ? NaN : Number(cleaned);
    if (!Number.isFinite(n) || n < 0) {
      delete scheduleNetSalesByWeekStore[mon][rid][iso];
      if (!Object.keys(scheduleNetSalesByWeekStore[mon][rid]).length) {
        delete scheduleNetSalesByWeekStore[mon][rid];
      }
      if (!Object.keys(scheduleNetSalesByWeekStore[mon]).length) {
        delete scheduleNetSalesByWeekStore[mon];
      }
    } else {
      scheduleNetSalesByWeekStore[mon][rid][iso] = String(Math.round(n * 100) / 100);
    }
    scheduleNetSalesByWeekStore = sanitizeScheduleNetSalesByWeek(scheduleNetSalesByWeekStore);
    persistScheduleNetSalesStore();
  }

  function loadScheduleNetSalesStore() {
    try {
      var raw = localStorage.getItem(SCHEDULE_NET_SALES_KEY);
      if (!raw) return {};
      return sanitizeScheduleNetSalesByWeek(JSON.parse(raw));
    } catch (_e) {
      return {};
    }
  }

  function persistScheduleNetSalesStore(opts) {
    try {
      localStorage.setItem(
        SCHEDULE_NET_SALES_KEY,
        JSON.stringify(sanitizeScheduleNetSalesByWeek(scheduleNetSalesByWeekStore || {}))
      );
      if (!(opts && opts.skipDirty) && GM_SUPABASE_DATA && window.gmSupabase) {
        draftScheduleDirty = true;
        scheduleTeamStateDebouncedSync();
  ...
  function absorbUnchangedRemoteNetSalesFromDraft(remoteDr) {
    ...
    try {
      ...
      if (confirmedRaw) {
        var confirmedObj = JSON.parse(confirmedRaw);
        confirmedSales = sanitizeScheduleNetSalesByWeek(
          confirmedObj && confirmedObj.scheduleNetSalesByWeek
        );
      }
    } catch (_c) {
      confirmedSales = {};
    }
    var remoteSales = sanitizeScheduleNetSalesByWeek(remotePayload.scheduleNetSalesByWeek);
    var localSales = sanitizeScheduleNetSalesByWeek(scheduleNetSalesByWeekStore);
    var localJson = JSON.stringify(localSales);
    var confirmedJson = JSON.stringify(confirmedSales);
    if (localJson !== confirmedJson) {
      var mergedDirty = mergeScheduleNetSalesByWeekMaps(localSales, remoteSales, 'local');
      if (JSON.stringify(mergedDirty) !== localJson) {
        scheduleNetSalesByWeekStore = mergedDirty;
        persistScheduleNetSalesStore({ skipDirty: true });
      }
      return;
    }
    var remoteJson = JSON.stringify(remoteSales);
    if (localJson === remoteJson) return;
    scheduleNetSalesByWeekStore = remoteSales;
    persistScheduleNetSalesStore({ skipDirty: true });
  }
  ...
  function getDraftScheduleRowsForWeek(weekIndex, restaurantId) {
    if (
      ...
      if (
        ...
        /* Return by reference — callers that mutate must cloneDraftSchedule first.
           Cloning here made every draftTimeSlotFor / rebuildSchedule catastrophically slow. */
        return scheduleTemplateEditorState.draft;
      }
    }
  ...
  function updateScheduleDownloadWeekButton(opts) {
    var btn = document.getElementById('scheduleDownloadWeekBtn');
    if (!btn) return;
    ...
    btn.textContent = busy
      ? gmT('schedule.downloadingWeek') || 'Downloading…'
      : gmT('schedule.downloadWeek') || 'Download';
    btn.title =
      gmT('schedule.downloadWeekTitle') ||
  ...
  async function downloadSelectedWeekScheduleExcel() {
    var wi = scheduleCalendarWeekIndex;
    var weekIso = mondayIsoForScheduleWeekIndex(wi);
    var range = formatScheduleWeekRangeLabel(wi);
    if (!weekIso) {
      showScheduleNotice(gmT('schedule.downloadWeekFailed') || 'Could not download this week’s schedule.', false);
      return { ok: false };
    }
    updateScheduleDownloadWeekButton({ busy: true });
    try {
      await ensureTimecardsManagerLoaded();
      var api = window.gmCalloutTimecards;
      if (!api || typeof api.downloadScheduleWeekOnly !== 'function') {
        showScheduleNotice(gmT('schedule.downloadWeekFailed') || 'Could not download this week’s schedule.', false);
        return { ok: false };
      }
      var result = await api.downloadScheduleWeekOnly({
      ...
      if (!result || !result.ok) {
        showScheduleNotice(
          (result && result.message) ||
            gmT('schedule.downloadWeekFailed') ||
            'Could not download this week’s schedule.',
          false
        );
        return { ok: false };
      }
      showScheduleNotice(
        gmT('schedule.downloadWeekDone', { range: range }) ||
          'Schedule downloaded for ' + range + '.',
        true
      );
      return { ok: true };
    } catch (err) {
      console.warn('downloadSelectedWeekScheduleExcel', err);
      showScheduleNotice(gmT('schedule.downloadWeekFailed') || 'Could not download this week’s schedule.', false);
      return { ok: false };
    } finally {
  ...
  async function publishSelectedWeekScheduleAndNotify(opts) {
    ...
    var range = formatScheduleWeekRangeLabel(wi);
    try {
      /* Always flush before notify — tile edits are debounced; Notify Again must not
         push employees to a stale cloud schedule. */
      if (teamStateHasDirtyFields() || newlyPublished || schedulePublishedDirty) {
  ...
  /** Delivery/dishwasher tip take-home factor (0–1), e.g. 0.95 = 95%. */
  function normalizeDeliveryTipRetention(val) {
    if (val == null || val === '') return null;
  ...
  function deliveryTipRetentionLocationId(emp) {
    ...
    if (loc === 'both') {
      ...
      loc = meta.primaryLocationId || meta.primaryRestaurantId || 'rp-9';
    }
    if (loc === 'rp-8' || loc === 'rp-9') return loc;
    return 'rp-9';
  }

  function defaultDeliveryTipRetentionForEmployee(emp) {
    if (!emp || emp.staffType !== 'Server') return null;
    if (isJuanEspinobarrosEmployee(emp)) return 0.95;
    return deliveryTipRetentionLocationId(emp) === 'rp-8' ? 0.8 : 0.95;
  }

  function deliveryTipRetentionFactorForEmployee(emp) {
    if (!emp || emp.staffType !== 'Server') return null;
    var raw =
      emp.deliveryTipRetention != null
        ...
        : emp.meta && emp.meta.deliveryTipRetention != null
          ...
          : null;
    var n = normalizeDeliveryTipRetention(raw);
    if (n != null) return n;
  ...
  function applyDeliveryTipRetentionDefaultIfMissing(emp) {
    if (!emp || emp.staffType !== 'Server') return;
    var existing =
      emp.deliveryTipRetention != null
        ? emp.deliveryTipRetention
        : emp.meta && emp.meta.deliveryTipRetention != null
          ? emp.meta.deliveryTipRetention
          : null;
    var n = normalizeDeliveryTipRetention(existing);
    if (n != null) {
      emp.deliveryTipRetention = n;
      emp.meta = emp.meta && typeof emp.meta === 'object' ? emp.meta : {};
      emp.meta.deliveryTipRetention = n;
      return;
    }
    var def = defaultDeliveryTipRetentionForEmployee(emp);
    if (def == null) return;
    emp.deliveryTipRetention = def;
    emp.meta = emp.meta && typeof emp.meta === 'object' ? emp.meta : {};
    emp.meta.deliveryTipRetention = def;
  }

  function applyDeliveryTipRetentionDefaultsToAllEmployees() {
    employees.forEach(applyDeliveryTipRetentionDefaultIfMissing);
  }
  ...
  function employeeRecordToDbRow(emp) {
    ...
    if (emp.staffType === 'Server') {
      var tipRet =
        emp.deliveryTipRetention != null
          ? normalizeDeliveryTipRetention(emp.deliveryTipRetention)
          : normalizeDeliveryTipRetention(meta.deliveryTipRetention);
      if (tipRet == null) tipRet = defaultDeliveryTipRetentionForEmployee(emp);
      if (tipRet != null) meta.deliveryTipRetention = tipRet;
      else if ('deliveryTipRetention' in meta) delete meta.deliveryTipRetention;
  ...
  function applyEmployeesFromRemoteDbRows(dbRows, opts) {
    ...
    applyDeliveryTipRetentionDefaultsToAllEmployees();
  ...
  function ensureTimecardsManagerLoaded() {
    ...
    timecardsManagerLoadPromise = new Promise(function (resolve, reject) {
      ...
      script.src =
        ...
        encodeURIComponent(
          (typeof window !== 'undefined' && window.__GM_ASSET_V) || 'perf-13'
        );
      script.async = true;
  ...
  /** Remove tip / leave-extra keys that reference a deleted employee id. */
  function scrubEmployeeFromTimecardLocalStores(empId) {
    if (!empId) return false;
    var id = String(empId);
    var changed = false;

    function scrubNestedWeekMap(all) {
      if (!all || typeof all !== 'object') return false;
      var did = false;
      Object.keys(all).forEach(function (weekKey) {
        var slice = all[weekKey];
        if (!slice || typeof slice !== 'object') return;
        Object.keys(slice).forEach(function (k) {
          if (k === id || k.indexOf(id + '@') === 0 || k.indexOf('|' + id + '|') >= 0 || k.indexOf(id + '|') === 0) {
            delete slice[k];
            did = true;
          }
        });
    ...
    try {
      var dw = loadTimecardDishwasherTipsStore();
      if (scrubNestedWeekMap(dw)) {
        localStorage.setItem(TIMECARD_DISHWASHER_TIPS_KEY, JSON.stringify(dw));
        changed = true;
      }
    } catch (_dwScrub) {
      /* ignore */
    ...
    try {
      var ex = loadTimecardWeekExtrasStore();
      if (scrubNestedWeekMap(ex)) {
        localStorage.setItem(TIMECARD_WEEK_EXTRAS_KEY, JSON.stringify(ex));
        changed = true;
      }
    } catch (_exScrub) {
      /* ignore */
    }
    if (changed) scheduleTipPayrollDebouncedSync();
    return changed;
  }
  ...
  try {
    ...
    if (bootGroupRaw) {
      ...
      bootConfirmedGroup = sanitizeGroupOrderPotentialByWeek(
        bootGroupObj && bootGroupObj.groupOrderPotentialByWeek
      );
  ...
  function hashScheduleBundle(assignments, draft) {
    var raw = '';
    try {
      raw = JSON.stringify(assignments == null ? null : assignments) + '\n' + JSON.stringify(draft == null ? null : draft);
    } catch (_e) {
      raw = String(Date.now());
    }
    var h = 2166136261;
  ...
  async function insertScheduleRevisionRow(opts) {
    ...
    var sb = window.gmSupabase;
    var teamStateId = gmCalloutTeamStateRowId();
    var assignments = opts.assignments != null ? opts.assignments : loadScheduleAssignmentsStore();
    var draft =
    ...
    try {
      ...
      try {
        ...
        uid = sess.data && sess.data.session && sess.data.session.user ? sess.data.session.user.id : null;
      } catch (_s) {
        uid = null;
      }
      var ins = await sb.from('team_state_schedule_revisions').insert({
        team_state_id: teamStateId,
        created_by: uid,
        source: source,
        label: opts.label || formatScheduleRevisionLabel(source),
        content_hash: contentHash,
        schedule_assignments: assignments || {},
        draft_schedule: draft || {},
        schedule_published: opts.published != null ? opts.published : null,
      });
      if (ins.error) {
  ...
  async function pruneScheduleRevisions(teamStateId) {
    if (!GM_SUPABASE_DATA || !window.gmSupabase) return;
    try {
      ...
      var ids = (res.data || []).map(function (r) {
        return r.id;
      }).filter(Boolean);
      if (!ids.length) return;
      await window.gmSupabase.from('team_state_schedule_revisions').delete().in('id', ids);
    } catch (_e) {
      /* ignore */
    }
  }
  ...
  /**
   * Refuse painting a stale/different remote schedule over a newer local push.
   * Other managers with a strictly newer updated_at still win after the echo window.
   */
  function shouldRefuseRemoteScheduleSnapshot(row) {
    if (!row || typeof row !== 'object') return false;
    if (teamStateScheduleBundleMergeLocked()) return true;
    var elapsed = Date.now() - (teamStateLastLocalPushAt || 0);
    ...
    if (
      ...
      (row.schedule_assignments != null || row.draft_schedule != null)
    ) {
      ...
      if (
        remoteHash !== scheduleLastPushHash &&
        (!remoteAt || !scheduleLastPushUpdatedAt || remoteAt <= scheduleLastPushUpdatedAt)
      ) {
        return true;
      }
    }
  ...
  function applyTeamStateRowFromRemoteInner(row, ctx) {
    ...
    var refuseStaleSchedule = shouldRefuseRemoteScheduleSnapshot(row);
        ...
        if (draftScheduleRemoteMergeIsStale(dr)) {
          ...
          if (remoteDraftPayload && remoteDraftPayload.byWeek) {
            ...
            try {
              ...
              groupOrderPotentialByWeekStore = mergedRemoteGroupOrder;
              scheduleNetSalesByWeekStore = mergedRemoteNetSales;
   ...
   * Pay = paidHours × hourlyRate — no tips / SoH (kept for callers; UI may omit $).
   * headcount = unique people actually working that day (excludes Unassigned and day-off cells).
   */
  function computeScheduleDayTotals(visibleDays) {
    ...
    SCHEDULE.forEach(function (shift) {
      ...
      /* Day-off = null draft cell — never count even if a stale SCHEDULE row remains. */
      var wk = weekdayKeyFromScheduleDay(shift.day);
      if (!draftTimeSlotFor(shift.role, wk, shift.trIdx, wi, currentRestaurantId)) return;
      ...
      workers.forEach(function (wname) {
        ...
        namesByDay[shift.day][String(wname).trim()] = true;
    ...
    Object.keys(byDay).forEach(function (dayStr) {
      byDay[dayStr].headcount = Object.keys(namesByDay[dayStr] || {}).length;
    });
    return byDay;
  }
  ...
  async function removeEmployeePhotoStorageObjects(empId) {
    if (!GM_SUPABASE_DATA || !window.gmSupabase || !empId || !isUuidCloudId(empId)) return;
    try {
      var paths = [empId + '.jpg', empId + '.jpeg', empId + '.png', empId + '.webp', empId + '.gif'];
      await window.gmSupabase.storage.from('employee-photos').remove(paths);
    } catch (_photoRm) {
      /* best-effort */
    }
  }

  function removePortalLocalAccountForEmployee(displayName) {
    if (!displayName) return;
    try {
      var accounts = loadPortalEmployeeAccounts();
      if (!Array.isArray(accounts) || !accounts.length) return;
      var loginKey = normPortalLoginKey(displayName);
      var next = accounts.filter(function (a) {
        if (!a) return false;
        if (a.loginKey && a.loginKey === loginKey) return false;
        if (a.displayName && workerNamesMatch(a.displayName, displayName)) return false;
        return true;
      });
      if (next.length !== accounts.length) savePortalEmployeeAccounts(next);
    } catch (_portalRm) {
      /* ignore */
    }
  }

  async function removeStaffRequestsForDeletedEmployee(displayName, empId) {
    var removedIds = [];
    for (var i = staffRequests.length - 1; i >= 0; i -= 1) {
      var r = staffRequests[i];
      if (!r) continue;
      var nameHit = r.employeeName && workerNamesMatch(r.employeeName, displayName);
      var swapHit =
        (empId && r.swapTargetEmployeeId && String(r.swapTargetEmployeeId) === String(empId)) ||
        (r.swapTargetEmployeeName && workerNamesMatch(r.swapTargetEmployeeName, displayName));
      if (!nameHit && !swapHit) continue;
      removedIds.push(r.id);
      staffRequests.splice(i, 1);
    }
    ...
    if (!GM_SUPABASE_DATA || !window.gmSupabase || !removedIds.length) return;
    try {
      for (var j = 0; j < removedIds.length; j += 1) {
        var id = removedIds[j];
        if (!id || String(id).indexOf('local-') === 0) continue;
        var res = await window.gmSupabase.from('staff_requests').delete().eq('id', id);
        if (res.error) console.warn('gm-callout: staff_requests delete', res.error);
      }
    } catch (err) {
      console.warn('gm-callout: staff_requests delete', err);
    }
  }
   ...
   */
  async function deleteEmployeeCompletely(emp) {
    if (!emp || !emp.id) return { ok: false, message: 'Employee not found.' };
    var empId = emp.id;
    var displayName = employeeDisplayName(emp);

    clearWorkerFromResolvedSchedule(displayName);
    scrubEmployeeFromTimecardLocalStores(empId);
    await scrubEmployeeTipExtrasOnTeamState(empId);
    await removeStaffRequestsForDeletedEmployee(displayName, empId);
    removePortalLocalAccountForEmployee(displayName);
    await removeEmployeePhotoStorageObjects(empId);

    if (GM_SUPABASE_DATA && window.gmSupabase && isUuidCloudId(empId)) {
      try {
        var delRes = await window.gmSupabase.from('employees').delete().eq('id', empId);
        if (delRes.error) {
          return {
            ok: false,
            message: delRes.error.message || 'Could not delete employee from cloud roster.',
          };
        }
      } catch (err) {
        return {
          ok: false,
          message: (err && err.message) || 'Could not delete employee from cloud roster.',
        };
      }
    }
    ...
    if (scheduleBody && typeof renderSchedule === 'function') renderSchedule();
    if (typeof renderEmployeeList === 'function') renderEmployeeList();
    if (typeof renderManagerAvailabilityScreen === 'function' && currentScreen === 13) {
      renderManagerAvailabilityScreen();
    }
    if (typeof renderManagerHomeShifts === 'function' && currentScreen === 14) {
      renderManagerHomeShifts();
    }
    notifyTimecardsEmployeesChanged();
    if (
      ...
      window.gmCalloutTimecards.onEmployeeDeleted(empId);
    }
    flushTeamStateSyncNow();
  ...
  async function scrubEmployeeTipExtrasOnTeamState(empId) {
    if (!GM_SUPABASE_DATA || !window.gmSupabase || !empId) return;
    try {
      var sb = window.gmSupabase;
      var res = await sb
        .from('team_state')
        .select('timecard_dishwasher_tips, timecard_week_extras')
        .eq('id', gmCalloutTeamStateRowId())
        .maybeSingle();
      if (res.error || !res.data) return;
      var dw = res.data.timecard_dishwasher_tips;
      var ex = res.data.timecard_week_extras;
      var patch = { id: gmCalloutTeamStateRowId() };
      var fields = [];
      var id = String(empId);

      function scrubMap(all) {
        if (!all || typeof all !== 'object') return false;
        var did = false;
        Object.keys(all).forEach(function (weekKey) {
          var slice = all[weekKey];
      ...
      if (dw && typeof dw === 'object') {
        var dwClone = JSON.parse(JSON.stringify(dw));
        if (scrubMap(dwClone)) {
          patch.timecard_dishwasher_tips = dwClone;
          fields.push('timecard_dishwasher_tips');
        }
      }
      if (ex && typeof ex === 'object') {
        var exClone = JSON.parse(JSON.stringify(ex));
        if (scrubMap(exClone)) {
          patch.timecard_week_extras = exClone;
          fields.push('timecard_week_extras');
        }
      }
      if (!fields.length) return;
      var up = await sb.from('team_state').upsert(patch, { onConflict: 'id' });
      if (up.error) console.warn('gm-callout: tip scrub on delete', up.error);
      else void broadcastTeamStateChanged(fields);
    } catch (err) {
      console.warn('gm-callout: tip scrub on delete', err);
    }
  }
  ...
  function migrateEmployeeRecord(e) {
    ...
    if (out.staffType === 'Server') {
      var tipRetRaw =
        e.deliveryTipRetention != null
          ? e.deliveryTipRetention
          : e.meta && e.meta.deliveryTipRetention != null
            ? e.meta.deliveryTipRetention
            : null;
      var tipRet = normalizeDeliveryTipRetention(tipRetRaw);
      if (tipRet == null) tipRet = defaultDeliveryTipRetentionForEmployee(out);
      if (tipRet != null) {
        out.deliveryTipRetention = tipRet;
        out.meta = out.meta && typeof out.meta === 'object' ? out.meta : {};
        out.meta.deliveryTipRetention = tipRet;
      }
  ...
  function saveScheduleAssignmentsStore(store, opts) {
    ...
    if (GM_SUPABASE_DATA && window.gmSupabase) scheduleAssignmentsDirty = true;
    scheduleTeamStateDebouncedSync();
    /* Interactive tile edits must stay local+debounced. Pass flushNow for publish / sign-out
       style paths that need an immediate cloud push. Compact-during-delete uses neither. */
  ...
  /** True only while the Templates modal owns scratch draft/assignments. */
  var scheduleTemplateScratchActive = false;
  /** When > 0, draft/assignment reads prefer live storage even if the template modal is open. */
  var schedulePreferLiveDataDepth = 0;

  function withLiveScheduleData(fn) {
    schedulePreferLiveDataDepth += 1;
    try {
      return fn();
    } finally {
      schedulePreferLiveDataDepth -= 1;
  ...
  function scheduleTemplateEditsAreScratch() {
    /* Require the active Templates session — leaked editor state must not hijack main schedule saves. */
    return (
      scheduleTemplateScratchActive &&
      !!scheduleTemplateEditorState &&
      schedulePreferLiveDataDepth === 0
  ...
  function captureScheduleTemplatePreviewSession() {
    ...
    var wi = scheduleCalendarWeekIndex;
    scheduleTemplatePreviewSession = withLiveScheduleData(function () {
      return {
        rid: rid,
        wi: wi,
  ...
  function restoreScheduleTemplatePreviewSession() {
    ...
    /* Prefer live reads — editor state may still be set until close clears it. */
  ...
  function applyTemplateEditorAssignmentsToScratch() {
    ...
    Object.keys(scheduleTemplateEditorState.workers || {}).forEach(function (rowKey) {
      ...
      var roleRows = draft[role];
      var row = roleRows && roleRows[trIdx];
      if (!row) return;
  ...
  function applyTemplateEditorStateToScratchOnly() {
    if (!scheduleTemplateEditorState) return;
    applyMasterTemplateChoicesToEditorDraft();
    applyTemplateEditorAssignmentsToScratch();
  }

  function restoreLiveScheduleAfterTemplatePreview() {
    withLiveScheduleData(function () {
      rebuildSchedule({
  ...
  function applyTemplateEditorStateToLiveScratch() {
    /* Scratch assignments/draft only — preview paint rebuilds SCHEDULE from that scratch. */
    applyTemplateEditorStateToScratchOnly();
  }

  var templateShiftEditorMountState = null;

  function closeTemplateShiftEditPanel(opts) {
    opts = opts || {};
    ...
    templateShiftEditorMountState = null;
    if (panel) panel.hidden = true;
    if (stage) stage.classList.remove('template-shift-edit-open');
    var ctx = document.getElementById('templateShiftEditContext');
    if (ctx) ctx.textContent = '';
    /*
     * Only clear shift targets when the in-template editor was actually open.
     * showScreen(2) always closes the Templates modal; wiping targets there made
     * main-schedule Save a no-op (no target → silent return).
     */
    if (!opts.keepTargets && wasMounted) {
      currentShift = null;
  ...
  function mountShiftEditorInTemplatePanel(contextText) {
    ...
    var saveBtn = document.getElementById('saveScheduleBtn');
    var actions = saveBtn && saveBtn.closest('.screen-actions');
    var titleEl = document.getElementById('templateShiftEditTitle');
    var ctx = document.getElementById('templateShiftEditContext');
    var liveCtx = document.getElementById('eligibleShiftContext');
    var stage =
      document.getElementById('scheduleTemplatePreviewStage') ||
      (panel && panel.closest('.schedule-template-preview-stage'));
    ...
    if (ctx) {
      ctx.textContent = contextText || (liveCtx && liveCtx.textContent) || '';
    }
    panel.hidden = false;
    if (stage) stage.classList.add('template-shift-edit-open');
    try {
      panel.scrollTop = 0;
    } catch (_scrollTpl) {
      /* ignore */
    }
    var closeBtn = document.getElementById('templateShiftEditClose');
  ...
  function presentShiftDetailEditor(contextText) {
    if (scheduleTemplateEditsAreScratch()) {
      mountShiftEditorInTemplatePanel(contextText);
      return;
    }
    showScreen(2);
  }
  ...
  function persistSingleShiftSlotEdit(role, trIdx, dayInWeek, start, end, breakText, isDayOff, opts) {
    ...
    if (templateScratch) {
      ...
      /* Template scratch must not rebuild live availability or double-paint:
         refreshScheduleCalendarAfterEdit → renderScheduleTemplatePreview does one rebuild. */
  ...
  function cloneScheduleUndoSnapshot() {
    return {
      ...
      groupOrderPotentialByWeek: sanitizeGroupOrderPotentialByWeek(
        JSON.parse(JSON.stringify(groupOrderPotentialByWeekStore || {}))
      ),
      scheduleNetSalesByWeek: sanitizeScheduleNetSalesByWeek(
        JSON.parse(JSON.stringify(scheduleNetSalesByWeekStore || {}))
      ),
    };
  }
  ...
  function clearCurrentWeekScheduleAll() {
    if (!managerCanEditCurrentRestaurant()) {
      showScheduleNotice(
        gmT('schedule.viewOnlyOtherStoreHint') || 'View only at this location.',
        false
      );
      return false;
    }
    ...
    try {
      ...
      ['Bartender', 'Kitchen', 'Server'].forEach(function (role) {
        var rows = Array.isArray(draft[role]) ? draft[role] : [];
        draft[role] = rows.map(function () {
          return makeNullDraftWeekRow();
        });
        if (!draft[role].length) {
          draft[role] = cloneDraftSchedule(DEFAULT_DRAFT_SCHEDULE_ROWS)[role].map(function () {
            return makeNullDraftWeekRow();
          });
        }
      });
      ...
      var store = loadScheduleAssignmentsStore();
      if (!store[rid]) store[rid] = {};
      resetRestaurantWeekDirectAssignments(store, rid, wi, {});
      saveScheduleAssignmentsStore(store);
      AVAILABILITY_SLOT_RANGES = buildAvailabilitySlotRangesUnion();
      pruneScheduleAssignmentsInvalidSlots();
      rebuildEmployeeDerivedData();
    ...
    showScheduleNotice('Cleared all people and shift times for ' + range + '.', false);
    return true;
  }
  ...
  function restoreScheduleUndoSnapshot(snap) {
    ...
    try {
      ...
      groupOrderPotentialByWeekStore = sanitizeGroupOrderPotentialByWeek(
        snap.groupOrderPotentialByWeek != null ? snap.groupOrderPotentialByWeek : {}
      );
      scheduleNetSalesByWeekStore = sanitizeScheduleNetSalesByWeek(
        snap.scheduleNetSalesByWeek != null ? snap.scheduleNetSalesByWeek : {}
      );
  ...
  function populateScheduleTemplateSelect(preferredId) {
    ...
    sel.innerHTML =
      '<option value="">This week’s schedule</option>' +
      list
        .map(function (t) {
          return (
        ...
        .join('');
    var pickId = preferredId != null ? preferredId : prev;
  ...
  function populateMasterScheduleTemplateSelect(preferredId) {
    ...
    var editSel = document.getElementById('masterScheduleTemplateSelect');
    if (editSel) {
      var editPrev = preferredId != null ? preferredId : editSel.value;
      editSel.innerHTML =
        '<option value="">Choose Master Template…</option>' + optionsHtml;
      if (editPrev && list.some(function (t) { return t.id === editPrev; })) {
        editSel.value = editPrev;
      } else {
        editSel.value = '';
      }
      var deleteBtn = document.getElementById('deleteMasterScheduleTemplateBtn');
      if (deleteBtn) deleteBtn.disabled = !editSel.value;
    }

    /* Guide dropdown is separate from the edit dropdown — keep both in sync. */
    var guideSel = document.getElementById('scheduleMasterTemplateSelect');
    if (guideSel) {
      var guidePrev = preferredId != null ? preferredId : guideSel.value;
      guideSel.innerHTML =
        '<option value="">No Master Template restriction</option>' + optionsHtml;
      if (guidePrev && list.some(function (t) { return t.id === guidePrev; })) {
        guideSel.value = guidePrev;
      } else if (
        scheduleTemplateEditorState &&
        scheduleTemplateEditorState.masterTemplateId &&
        ...
        guideSel.value = scheduleTemplateEditorState.masterTemplateId;
      } else {
        guideSel.value = '';
      }
    }
  ...
  function lookupScheduleAssignment(stored, shiftId) {
    ...
    var tplStart = SCHEDULE_TEMPLATE_WEEK_INDEX * 7;
    /*
     * Past weeks must not inherit staffing from the rolling "this week" pattern.
     * That path created duplicate people (e.g. second Maeve) when this week's layout
     * grew and past weeks briefly shared structure via draft fallback.
     */
  ...
  function saveScheduleAssignments(opts) {
    opts = opts || {};
    /* Never stamp the live assignment store from in-memory SCHEDULE while the
       Templates preview owns SCHEDULE as a scratch canvas. */
    if (scheduleTemplateEditsAreScratch() && !opts.forceDuringTemplate) {
      return;
    }
    ...
    var weekOnly =
      opts.weekIndex != null && !isNaN(Number(opts.weekIndex))
        ...
        : scheduleCalendarWeekIndex;
    var weekStart = resolveDraftWeekIndex(weekOnly) * 7;
    var weekEnd = weekStart + 7;
    SCHEDULE.forEach(function (s) {
      if (!s || !s.id) return;
  ...
  function employeeMatchesTeamRestaurant(emp, restaurantId) {
    if (!emp) return false;
    var u = emp.usualRestaurant || 'both';
    if (u === 'both') return true;
    return u === restaurantId;
  ...
  /** True when this employee is week-borrowed TO restaurantId for the visible schedule week. */
  function employeeIsWeekBorrowedToRestaurant(emp, restaurantId) {
    if (!emp || !restaurantId) return false;
    ...
    return !!(borrowedTo && borrowedTo === restaurantId);
  }

  /**
   * Home / both / week-borrow for this store. Used for visibility and assignment checks.
   * Person dropdowns use employeeMatchesTeamRestaurant so a one-time borrow does not
   * keep single-home staff on every other-store picker.
   */
  function employeeMatchesScheduleRestaurant(emp, restaurantId) {
    if (!emp) return false;
    if (employeeMatchesTeamRestaurant(emp, restaurantId)) return true;
   ...
   *  Also pause main-calendar remote refresh while Templates owns SCHEDULE as scratch. */
  function calendarScheduleUiBlocksRender() {
    return (
      calendarInlineWorkerEditIsOpen() ||
      calendarPersonSelectIsOpen() ||
      scheduleTemplateScratchActive
    );
  }
  ...
  function closeScheduleTemplateModal(opts) {
    ...
    if (opts.keepLiveSchedule) {
      scheduleTemplatePreviewSession = null;
      /* Apply already wrote live storage + rebuilt; drop scratch without reverting. */
    ...
    var applyConfirm = document.getElementById('applyScheduleTemplateConfirm');
    var deleteConfirm = document.getElementById('deleteScheduleTemplateConfirm');
    if (applyConfirm) applyConfirm.hidden = true;
    if (deleteConfirm) deleteConfirm.hidden = true;
    window.__gmPendingDeleteTemplateId = '';
    window.__gmPendingDeleteTemplateName = '';
  ...
  function renderScheduleTemplatePreview() {
    ...
    applyTemplateEditorStateToScratchOnly();
    /* Paint from template scratch into SCHEDULE for the preview grid only.
       Live draft/assignments stay in scheduleTemplatePreviewSession until Apply/close. */
    rebuildSchedule({
      weekIndex: scheduleCalendarWeekIndex,
      preserveOtherWeeks: true,
      skipRebind: true,
    ...
    mount.innerHTML = '';
    renderCalendarInto(mount, {
      readOnly: false,
      showDayTotals: true,
      force: true,
      /* Cross-store tile flags are live schedule noise — not part of the template. */
      showOtherStoreBadges: false,
      /* Preview mount — do not run main-grid interaction / deferred refresh side effects. */
  ...
  function setMasterScheduleTemplateNameFieldVisible(visible) {
    var field = document.getElementById('masterScheduleTemplateNameField');
  ...
  function renderMasterScheduleTemplateEditor() {
    ...
    setMasterScheduleTemplateNameFieldVisible(true);
          ...
          rows
            .map(function (row, index) {
              return (
                ...
                index +
                '">' +
                '<td><input class="timecards-input schedule-master-role" data-master-field="role" value="' +
                escapeHtml(row.role) +
                '" placeholder="Role" /></td>' +
                '<td><input class="timecards-input schedule-master-shift" data-master-field="shift" value="' +
                ...
                '</td>' +
                '<td><input class="timecards-input schedule-master-days" data-master-field="daysPerWeek" value="' +
                escapeHtml(row.daysPerWeek) +
                '" placeholder="Days" inputmode="numeric" /></td>' +
  ...
  function openScheduleTemplateModal() {
    ...
    scheduleTemplateEditorState.sourceTemplateId = '';
    ...
    setMasterScheduleTemplateNameFieldVisible(false);
    var masterNameInput = document.getElementById('masterScheduleTemplateNameInput');
    ...
    scheduleMasterEditorState = null;
    var applyConfirmOpen = document.getElementById('applyScheduleTemplateConfirm');
    var deleteConfirmOpen = document.getElementById('deleteScheduleTemplateConfirm');
    if (applyConfirmOpen) applyConfirmOpen.hidden = true;
    if (deleteConfirmOpen) deleteConfirmOpen.hidden = true;
    window.__gmPendingDeleteTemplateId = '';
    window.__gmPendingDeleteTemplateName = '';
    closeTemplateShiftEditPanel();
    renderScheduleTemplatePreview();
  ...
  /**
   * Replace only one week’s assignment keys for a restaurant. Other weeks stay untouched.
   */
  function mergeRestaurantWeekAssignmentsFromScratch(liveRs, scratchRs, weekIndex) {
    var weekStart = resolveDraftWeekIndex(weekIndex) * 7;
    var weekEnd = weekStart + 7;
    var next = liveRs && typeof liveRs === 'object' ? JSON.parse(JSON.stringify(liveRs)) : {};
    Object.keys(next).forEach(function (shiftId) {
      var p = parseShiftIdParts(shiftId);
      if (!p) return;
      if (p.globalDayIdx >= weekStart && p.globalDayIdx < weekEnd) delete next[shiftId];
    });
    Object.keys(scratchRs || {}).forEach(function (shiftId) {
      var p = parseShiftIdParts(shiftId);
      if (!p) return;
      if (p.globalDayIdx < weekStart || p.globalDayIdx >= weekEnd) return;
      next[shiftId] = JSON.parse(JSON.stringify(scratchRs[shiftId]));
    });
    return next;
  }

  function commitScheduleTemplateEditorToLive() {
    if (!managerCanEditCurrentRestaurant()) {
      return { ok: false, reason: 'view_only', appliedSlots: 0 };
    }
    if (!scheduleTemplateEditorState) {
      return { ok: false, reason: 'no_editor', appliedSlots: 0 };
    }
    var rid = currentRestaurantId;
    var wi = scheduleCalendarWeekIndex;
    applyMasterTemplateChoicesToEditorDraft();
    ...
    var appliedSlots = 0;
    try {
      /* Draft times: this week only. */
      saveDraftScheduleRowsForWeek(wi, scheduleTemplateEditorState.draft, rid);
      var visibleDays = getVisibleWeekDays();
      var weekPattern = {};
      Object.keys(scheduleTemplateEditorState.workers || {}).forEach(function (rowKey) {
        var bits = rowKey.split(':');
        ...
        for (var dayIndex = 0; dayIndex < visibleDays.length; dayIndex += 1) {
          ...
          weekPattern[key] = { workers: [worker] };
          appliedSlots += 1;
        }
      });
      if (scheduleTemplateEditorState.assignmentScratch) {
        /*
         * Templates are per-location and per-week. Merge only this restaurant’s
         * current-week shift keys so Apply cannot rewrite sibling weeks (or
         * freeze inherited names onto other weeks).
         */
        var liveStore = loadScheduleAssignmentsStore();
        var nextStore = JSON.parse(JSON.stringify(liveStore));
        var scratchRs = scheduleTemplateEditorState.assignmentScratch[rid] || {};
        resetRestaurantWeekDirectAssignments(nextStore, rid, wi, weekPattern);
        nextStore[rid] = mergeRestaurantWeekAssignmentsFromScratch(
          nextStore[rid],
          scratchRs,
          wi
        );
      ...
      notifyTimecardsScheduleChanged();
      scheduleTeamStateDebouncedSync();
      if (!appliedSlots && weekPatternHasStaffedSlots(weekPattern)) {
        appliedSlots = Object.keys(weekPattern).length;
      }
    } finally {
      scheduleUndoSuppressPush = false;
    }
    return { ok: true, appliedSlots: appliedSlots };
  }

  function resetScheduleTemplateEditorToCurrentWeek() {
    scheduleTemplateEditorState = currentTemplateEditorState();
    scheduleTemplateEditorState.masterTemplateId = '';
    scheduleTemplateEditorState.choices = {};
    scheduleTemplateEditorState.sourceTemplateId = '';
  ...
  function saveMasterScheduleTemplateFromEditor() {
    if (!managerCanEditCurrentRestaurant() || !scheduleMasterEditorState) {
      return { ok: false, reason: 'unavailable' };
    }
    var nameInput = document.getElementById('masterScheduleTemplateNameInput');
    var nameField = document.getElementById('masterScheduleTemplateNameField');
    ...
    if (!name) {
      if (nameInput) {
        ...
        nameInput.classList.add('schedule-template-name-input--invalid');
        try {
          nameInput.focus();
    ...
    if (nameInput) {
      ...
      nameInput.classList.remove('schedule-template-name-input--invalid');
    }
    ...
    return { ok: true, id: scheduleMasterEditorState.id };
  }
  ...
  var SCHEDULE_BORROW_PERSON_VALUE = '__gm_borrow_employee__';

  /** Pay-week bounds for the week shown on the schedule (for week-borrow extras). */
  function scheduleVisibleWeekPayBounds() {
    var tc = window.gmCalloutTimecards;
    try {
      var weekMon = mondayIsoForScheduleWeekIndex(scheduleCalendarWeekIndex);
      if (weekMon && tc && typeof tc.payWeekBoundsFromMonday === 'function') {
        return tc.payWeekBoundsFromMonday(new Date(weekMon + 'T12:00:00'));
      }
    } catch (_eBounds) {
      /* ignore */
    }
  ...
  /** Other-store staff for this role (Borrow employee picker). */
  function employeesForScheduleBorrowPicker(role) {
    var seen = Object.create(null);
    var out = [];
    employees
      .filter(function (e) {
        ...
        if (home === 'both' || home === currentRestaurantId) return false;
  ...
  function scheduleBorrowHomeMetaLabel(emp) {
    var u = emp && emp.usualRestaurant ? emp.usualRestaurant : 'both';
    if (u === 'both') return gmT('schedule.borrowHomeBoth');
    return gmT('schedule.borrowHomeStore', { store: restaurantShortLabel(u) });
  }

  /**
   * Mark single-home staff as borrowed TO the current store for this schedule's pay week.
   * Permanent Team "both" staff are not week-borrowed (company-wide OT stays).
   */
  function ensureScheduleWeekBorrowForEmployee(emp) {
    if (!emp) return;
    var tc = window.gmCalloutTimecards;
    if (
      !tc ||
      typeof tc.employeeEligibleForWeekBorrow !== 'function' ||
      typeof tc.setEmployeeBorrowedRestaurant !== 'function'
    ) {
      return;
    }
    if (!tc.employeeEligibleForWeekBorrow(emp)) return;
    var home = emp.usualRestaurant || 'both';
    ...
    tc.setEmployeeBorrowedRestaurant(emp.id, currentRestaurantId, scheduleVisibleWeekPayBounds());
  }

  var scheduleBorrowModalTarget = null;

  function closeScheduleBorrowEmployeeModal() {
    var modal = document.getElementById('scheduleBorrowEmployeeModal');
    if (modal) {
      modal.hidden = true;
      modal.setAttribute('aria-hidden', 'true');
    }
    scheduleBorrowModalTarget = null;
  }

  function openScheduleBorrowEmployeeModal(role, trIdx) {
    if (!managerCanEditCurrentRestaurant()) return;
    var modal = document.getElementById('scheduleBorrowEmployeeModal');
    var listEl = document.getElementById('scheduleBorrowEmployeeList');
    if (!modal || !listEl) return;
    scheduleBorrowModalTarget = { role: role, trIdx: trIdx };
    var people = employeesForScheduleBorrowPicker(role);
    if (!people.length) {
      listEl.innerHTML =
        '<p class="schedule-borrow-employee-empty">' +
        escapeHtml(gmT('schedule.borrowEmpty')) +
        '</p>';
    } else {
      listEl.innerHTML = people
        .map(function (emp) {
          ...
          return (
            '<button type="button" class="schedule-borrow-employee-btn" role="option" data-borrow-emp-id="' +
            escapeHtml(emp.id) +
            '" data-borrow-emp-name="' +
            escapeHtml(name) +
            '">' +
            '<span class="schedule-borrow-employee-name">' +
            escapeHtml(name) +
            '</span>' +
            '<span class="schedule-borrow-employee-meta">' +
            escapeHtml(scheduleBorrowHomeMetaLabel(emp)) +
            '</span>' +
            '</button>'
          );
        })
  ...
  function ensureScheduleBorrowEmployeeModalWired() {
    var modal = document.getElementById('scheduleBorrowEmployeeModal');
    if (!modal || modal.dataset.gmBorrowWired === '1') return;
    modal.dataset.gmBorrowWired = '1';
    function onDismiss() {
      closeScheduleBorrowEmployeeModal();
    }
    var backdrop = document.getElementById('scheduleBorrowEmployeeModalBackdrop');
    var closeBtn = document.getElementById('scheduleBorrowEmployeeModalClose');
    var cancelBtn = document.getElementById('scheduleBorrowEmployeeCancel');
    ...
    if (listEl) {
      listEl.addEventListener('click', function (e) {
        var btn = e.target.closest('.schedule-borrow-employee-btn');
        if (!btn || !scheduleBorrowModalTarget) return;
        var empId = btn.getAttribute('data-borrow-emp-id');
        var name = btn.getAttribute('data-borrow-emp-name');
        var role = scheduleBorrowModalTarget.role;
        var trIdx = scheduleBorrowModalTarget.trIdx;
        closeScheduleBorrowEmployeeModal();
        var emp = empId
          ? employees.find(function (e) {
              return e && e.id === empId;
            })
          : employeeByDisplayName(name);
        var finish = function () {
          ensureScheduleWeekBorrowForEmployee(emp);
          assignPersonToScheduleRow(role, trIdx, name || 'Unassigned');
        };
        if (window.gmCalloutEnsureTimecardsManagerLoaded) {
          window.gmCalloutEnsureTimecardsManagerLoaded().then(finish).catch(finish);
        } else {
          finish();
        }
      });
  ...
  function otherStoreDayLabelKey(workerName, dayStr) {
  ...
  function otherStoreLabelFromMap(map, workerName, dayStr) {
    if (!map || !workerName || workerName === 'Unassigned') return '';
    return map[otherStoreDayLabelKey(workerName, dayStr)] || '';
  }

  function calendarOtherStoreBadgeHtml(label) {
    if (!label) return '';
    return (
      '<div class="calendar-slot-other-store" title="' +
      escapeHtml(label) +
      '">' +
      escapeHtml(label) +
      '</div>'
  ...
  function renderCalendarInto(targetEl, opts) {
    ...
    const showOtherStoreBadges = opts.showOtherStoreBadges !== false;
    const otherStoreDayLabels = showOtherStoreBadges
    ...
    var totalsHeader = showPersonTotals
      ? '<th scope="col" class="calendar-person-totals-col">' +
        ...
        escapeHtml(gmT('schedule.personTotals') || 'Totals') +
        '</span>' +
        '<div class="calendar-th-date-sub">' +
        escapeHtml(gmT('schedule.personTotalsSub') || 'Week') +
        '</div>' +
        '</th>'
      : '';
    var headerHtml =
    ...
    function pushSectionRow(roleKey, sectionClass) {
      bodyRows.push(
        '<tr class="calendar-group-row ' +
          sectionClass +
          '">' +
          '<td class="time-col calendar-row-person-col calendar-group-label">' +
          escapeHtml(gmStaffTypeLabel(roleKey)) +
          (readOnly
            ? ''
            : '<span class="calendar-section-actions"><button type="button" class="calendar-add-slot-btn" data-add-slot-role="' +
              escapeHtml(roleKey) +
              '">' +
        ...
        const tds = visibleDays
          .map(function (dayStr) {
            ...
            const slotLabel =
              ...
              ', ' +
              rpTime +
              (otherLblShift ? ', also ' + otherLblShift : '') +
              '.';
            const slotTitle = readOnly
              ? ''
              : ' title="Click to edit · × or Option/Alt+Delete for day off"';
            ...
            return (
              ...
              (readOnly
                ? ''
                : '<button type="button" class="calendar-slot-dayoff-btn" data-calendar-dayoff="1" title="' +
                  escapeHtml(gmT('schedule.markDayOff') || 'Mark day off') +
                  '" aria-label="' +
                  escapeHtml(gmT('schedule.markDayOff') || 'Mark day off') +
                  '">&times;</button>') +
    ...
    syncCalendarTheadStickyOffset(targetEl);
  }

  function schedulePanelOpenFromStorage(key) {
    try {
  ...
  function setSchedulePanelOpenStorage(key, open) {
    try {
  ...
  function ensureScheduleBelowCalendarHost() {
    if (!calendarGrid) return null;
    var host = document.getElementById('scheduleBelowCalendar');
    if (!host) {
      host = document.createElement('div');
      host.id = 'scheduleBelowCalendar';
      host.className = 'schedule-below-calendar';
    }
    /* Inside #calendarGrid so panels scroll with the matrix (not pinned under it). */
    if (host.parentNode !== calendarGrid) {
      calendarGrid.appendChild(host);
    }
    return host;
  }

  function formatScheduleLaborPct(pay, salesStr) {
    var sales = Number(String(salesStr || '').replace(/[$,\s]/g, ''));
    if (!Number.isFinite(sales) || sales <= 0) return '—';
    var pct = ((Number(pay) || 0) / sales) * 100;
    if (!Number.isFinite(pct)) return '—';
    return pct.toFixed(1) + '%';
  }

  function scheduleManagerBelowPanelsAllowed() {
    return (
      document.documentElement.classList.contains('manager-app') ||
      document.documentElement.classList.contains('admin-app')
    );
  }

  function renderScheduleManagerBelowPanels(visibleDays, showPersonTotals, readOnly) {
    var host = ensureScheduleBelowCalendarHost();
    if (!host) return;
    if (!scheduleManagerBelowPanelsAllowed()) {
      host.hidden = true;
      host.innerHTML = '';
      return;
    }
    ...
    function weekTotalsTd(labelHtml) {
      if (!showPersonTotals) return '';
      return (
        '<td class="schedule-panel-week-totals">' +
        '<div class="schedule-panel-readonly">' +
        escapeHtml(labelHtml) +
        '</div></td>'
      );
    }

    function parsePanelNumber(raw) {
    ...
    (visibleDays || []).forEach(function (d) {
      ...
      var salesRaw = getScheduleNetSalesCell(currentRestaurantId, weekMon, dayIso);
      if (salesRaw !== '') {
        weekHasSales = true;
        var salesN = parsePanelNumber(salesRaw);
        if (Number.isFinite(salesN)) weekSales += salesN;
      }
    });
    SCHEDULE.forEach(function (shift) {
      if (!shift || (visibleDays || []).indexOf(shift.day) < 0) return;
      var wk = weekdayKeyFromScheduleDay(shift.day);
      if (!draftTimeSlotFor(shift.role, wk, shift.trIdx, wiLabor, currentRestaurantId)) return;
      var workers = (shift.workers || [shift.worker].filter(Boolean)).filter(function (n) {
        return n && n !== 'Unassigned';
      });
      if (!workers.length) return;
      if (scheduleShiftGrossHours(shift) <= 0) return;
      workers.forEach(function (wname) {
        weekNames[String(wname).trim()] = true;
      });
    });
    var weekHeadcount = Object.keys(weekNames).length;
    var weekLaborPct = formatScheduleLaborPct(weekPay, weekHasSales ? String(weekSales) : '');

    function laborReadonlyCells(getter) {
      return visibleDays
        .map(function (dayStr) {
          return (
            '<td class="schedule-panel-cell">' +
            '<div class="schedule-panel-readonly">' +
            escapeHtml(getter(dayStr)) +
            '</div></td>'
          );
        })
        .join('');
    ...
    function netSalesCells() {
      return visibleDays
        .map(function (dayStr) {
          ...
          return (
            '<td class="schedule-panel-cell">' +
            '<input type="text" inputmode="decimal" class="schedule-panel-input schedule-net-sales-input" maxlength="16" autocomplete="off" ' +
            'placeholder="0" data-net-sales-day-iso="' +
            escapeHtml(dayIso) +
            '" aria-label="' +
            escapeHtml((gmT('schedule.netSales') || 'Net sales') + ' · ' + dayStr) +
            '" value="' +
            escapeHtml(val) +
            '" />' +
            '</td>'
    ...
    var laborBody =
      ...
      '<tr class="schedule-panel-data-row">' +
      '<th scope="row" class="schedule-panel-row-label">' +
      escapeHtml(gmT('schedule.headcount') || 'Headcount') +
      '</th>' +
      laborReadonlyCells(function (d) {
        return String((dayTotals[d] && dayTotals[d].headcount) || 0);
      }) +
      weekTotalsTd(String(weekHeadcount)) +
      '</tr>' +
      '<tr class="schedule-panel-data-row">' +
      '<th scope="row" class="schedule-panel-row-label">' +
      escapeHtml(gmT('schedule.netSales') || 'Net sales') +
      '</th>' +
      netSalesCells() +
      weekTotalsTd(weekHasSales ? String(Math.round(weekSales * 100) / 100) : '—') +
      '</tr>' +
      '<tr class="schedule-panel-data-row">' +
      '<th scope="row" class="schedule-panel-row-label">' +
      ...
      '<tr class="schedule-panel-data-row">' +
      '<th scope="row" class="schedule-panel-row-label">' +
      escapeHtml(gmT('schedule.laborPct') || 'Labor %') +
      '</th>' +
      laborReadonlyCells(function (d) {
        var meta = WEEK_META.find(function (m) {
          return m.label === d;
    ...
    var groupRows = GROUP_ORDER_POTENTIAL_PLATFORMS.map(function (plat) {
      var weekSum = 0;
      ...
      var cells = visibleDays
        .map(function (dayStr) {
          ...
          return (
            '<td class="schedule-panel-cell">' +
            '<input type="text" inputmode="decimal" class="schedule-panel-input calendar-group-order-input" maxlength="80" autocomplete="off" ' +
            'placeholder="0" data-group-order-platform="' +
            escapeHtml(plat.id) +
            '" data-group-order-day-iso="' +
            escapeHtml(dayIso) +
            '" aria-label="' +
            escapeHtml(plat.label + ' · ' + dayStr) +
            '" value="' +
            escapeHtml(val) +
            '" />' +
            '</td>'
        ...
        .join('');
      var weekLabel = weekHasNum ? String(Math.round(weekSum * 100) / 100) : '0';
      return (
        '<tr class="schedule-panel-data-row" data-group-order-platform="' +
        escapeHtml(plat.id) +
        '">' +
        '<th scope="row" class="schedule-panel-row-label">' +
        escapeHtml(plat.label) +
        '</th>' +
        cells +
        weekTotalsTd(weekLabel) +
        '</tr>'
      );
    }).join('');

    host.innerHTML =
      '<div class="schedule-collapsible' +
      (laborOpen ? ' is-open' : '') +
      '" data-schedule-panel="labor">' +
      '<button type="button" class="schedule-collapsible-toggle" aria-expanded="' +
      (laborOpen ? 'true' : 'false') +
      '" data-schedule-panel-toggle="labor">' +
      '<span class="schedule-collapsible-chevron" aria-hidden="true"></span>' +
      '<span class="schedule-collapsible-title">' +
      escapeHtml(gmT('schedule.laborSalesPanel') || 'Labor & sales') +
      '</span></button>' +
      '<div class="schedule-collapsible-body"' +
      (laborOpen ? '' : ' hidden') +
      '>' +
      '<div class="schedule-panel-scroll">' +
      '<table class="schedule-panel-matrix' +
      (showPersonTotals ? ' schedule-panel-matrix--person-totals' : '') +
      '"><tbody>' +
      laborBody +
      '</tbody></table></div></div></div>' +
      '<div class="schedule-collapsible' +
      (groupOpen ? ' is-open' : '') +
      '" data-schedule-panel="group">' +
      '<button type="button" class="schedule-collapsible-toggle" aria-expanded="' +
      (groupOpen ? 'true' : 'false') +
      '" data-schedule-panel-toggle="group">' +
      '<span class="schedule-collapsible-chevron" aria-hidden="true"></span>' +
      '<span class="schedule-collapsible-title">' +
      escapeHtml(gmT('schedule.groupOrderPotential') || 'GROUP ORDER / POTENTIAL') +
      '</span></button>' +
      '<div class="schedule-collapsible-body"' +
      (groupOpen ? '' : ' hidden') +
      '>' +
  ...
  /**
   * Copy Person / day / totals column widths from the main calendar thead
   * so labor & group-order day cells line up under schedule tiles.
   */
  function syncSchedulePanelColumnAlignment() {
    ...
    var widths = [];
    var total = 0;
    for (var i = 0; i < srcThs.length; i += 1) {
      var w = Math.round(srcThs[i].getBoundingClientRect().width);
      if (!(w > 0)) return;
      widths.push(w);
      total += w;
    }
    /* Host defaults to the scrollport width; pin it to the matrix so Sat/Sun aren’t clipped. */
    var matrixW = Math.max(total, Math.round(matrix.offsetWidth || 0), Math.round(matrix.scrollWidth || 0));
    host.style.width = matrixW + 'px';
    host.style.minWidth = matrixW + 'px';
    host.querySelectorAll('.schedule-collapsible').forEach(function (el) {
      el.style.width = matrixW + 'px';
      el.style.minWidth = matrixW + 'px';
    });
    host.querySelectorAll('table.schedule-panel-matrix').forEach(function (table) {
      ...
      table.querySelectorAll('tr').forEach(function (tr) {
        ...
        for (var c = 0; c < cells.length && c < widths.length; c += 1) {
          cells[c].style.width = widths[c] + 'px';
          cells[c].style.minWidth = widths[c] + 'px';
          cells[c].style.maxWidth = widths[c] + 'px';
  ...
  function ensureScheduleBelowCalendarInteraction() {
    ...
    host.addEventListener('click', function (e) {
      ...
      if (!toggle) return;
      e.preventDefault();
      var which = toggle.getAttribute('data-schedule-panel-toggle');
      var panel = toggle.closest('.schedule-collapsible');
      if (!panel) return;
      var body = panel.querySelector('.schedule-collapsible-body');
      var open = !panel.classList.contains('is-open');
      panel.classList.toggle('is-open', open);
      if (body) body.hidden = !open;
      toggle.setAttribute('aria-expanded', open ? 'true' : 'false');
      setSchedulePanelOpenStorage(
        which === 'labor' ? SCHEDULE_LABOR_PANEL_OPEN_KEY : SCHEDULE_GROUP_PANEL_OPEN_KEY,
        open
      );
      if (open) {
        requestAnimationFrame(function () {
    ...
    host.addEventListener('change', function (e) {
      ...
      if (groupInp) {
        ...
        if (!platformId || !dayIso || !weekMon) return;
  ...
  function syncCalendarTheadStickyOffset(rootEl) {
    if (!rootEl) return;
    var apply = function () {
      var ths = rootEl.querySelectorAll('.calendar-matrix thead th');
  ...
  var calendarTheadResizeBound = false;
  function ensureCalendarTheadStickyOffsetOnResize() {
    if (calendarTheadResizeBound) return;
    calendarTheadResizeBound = true;
    var timer = null;
    window.addEventListener('resize', function () {
      ...
      timer = setTimeout(function () {
        ...
        syncCalendarTheadStickyOffset(document.getElementById('empCalendarGrid'));
        syncSchedulePanelColumnAlignment();
      }, 80);
    });
  }
  ...
  function ensureCalendarInteraction() {
    ...
    calendarGrid.addEventListener('keydown', function (e) {
      if (!managerCanEditCurrentRestaurant()) return;
      if (e.target.closest('.calendar-row-person-select, .calendar-cell-name-input, .calendar-group-order-input')) return;
  ...
  /** Alphabetical by display name (Team page). */
  ...
  function normalizeEmployeePosition(emp) {
    var raw =
      emp && emp.meta && emp.meta.position != null ? String(emp.meta.position).trim() : '';
    return raw.replace(/\s+/g, ' ').toLowerCase();
  }

  /** Store Manager (0) then Kitchen Manager (1); others (2) follow seniority. */
  function employeeManagerPinRank(emp) {
    var pos = normalizeEmployeePosition(emp);
  ...
  /**
   * Within a role group: managers first, then hire date ascending (most senior first);
   * missing hire date → after dated; ties / no-date → alphabetical by first name.
   */
  function compareEmployeesBySeniority(a, b) {
    var pa = employeeManagerPinRank(a);
    var pb = employeeManagerPinRank(b);
  ...
  const empDeliveryTipRetention = document.getElementById('empDeliveryTipRetention');
  const empDeliveryTipRetentionWrap = document.getElementById('empDeliveryTipRetentionWrap');
  ...
  function openEmployeeForm(empId) {
    ...
    syncEmployeeDeliveryTipRetentionField(emp);
  ...
  function openShiftEditForSlot(role, trIdx, dayStr) {
    ...
    presentShiftDetailEditor(
      eligibleShiftContext ? eligibleShiftContext.textContent : ''
    );
  }
  ...
  var scheduleClearAllBtn = document.getElementById('scheduleClearAllBtn');
  var scheduleClearAllConfirm = document.getElementById('scheduleClearAllConfirm');
  var scheduleClearAllConfirmCancel = document.getElementById('scheduleClearAllConfirmCancel');
  var scheduleClearAllConfirmYes = document.getElementById('scheduleClearAllConfirmYes');
  var scheduleClearAllConfirmMsg = document.getElementById('scheduleClearAllConfirmMsg');
  if (scheduleClearAllBtn) {
    scheduleClearAllBtn.addEventListener('click', function () {
      if (!managerCanEditCurrentRestaurant()) {
        showScheduleNotice(
          gmT('schedule.viewOnlyOtherStoreHint') || 'View only at this location.',
          false
      ...
      var range = formatScheduleWeekRangeLabel(scheduleCalendarWeekIndex);
      if (scheduleClearAllConfirmMsg) {
        scheduleClearAllConfirmMsg.textContent =
          'Clear all people and shift times for ' +
          range +
          ' at this location? This cannot be undone except with Undo.';
      }
      if (scheduleClearAllConfirm) {
        scheduleClearAllConfirm.hidden = false;
        if (scheduleClearAllConfirmYes) {
          setTimeout(function () {
            scheduleClearAllConfirmYes.focus();
          }, 0);
        }
        return;
      }
      ...
      clearCurrentWeekScheduleAll();
    });
  }
  if (scheduleClearAllConfirmCancel) {
    scheduleClearAllConfirmCancel.addEventListener('click', function () {
      if (scheduleClearAllConfirm) scheduleClearAllConfirm.hidden = true;
    });
  }
  if (scheduleClearAllConfirmYes) {
    scheduleClearAllConfirmYes.addEventListener('click', function () {
      if (scheduleClearAllConfirm) scheduleClearAllConfirm.hidden = true;
      clearCurrentWeekScheduleAll();
    });
  }
  ...
  async function openScheduleHistoryModal() {
    if (!scheduleHistoryModal || !managerCanEditCurrentRestaurant()) return;
    scheduleHistoryModal.hidden = false;
    if (scheduleHistoryList) {
      scheduleHistoryList.innerHTML = '<p class="calendar-hint">' + escapeHtml(gmT('common.loading') || 'Loading…') + '</p>';
    }
    if (!GM_SUPABASE_DATA || !window.gmSupabase) {
      if (scheduleHistoryList) {
        scheduleHistoryList.innerHTML =
          '<p class="calendar-hint">' + escapeHtml(gmT('schedule.historyFailed') || 'Could not load history.') + '</p>';
      }
      return;
    }
    var res = await window.gmSupabase
    ...
    if (res.error) {
      if (scheduleHistoryList) {
        scheduleHistoryList.innerHTML =
          ...
          escapeHtml(res.error.message || gmT('schedule.historyFailed') || 'Could not load history.') +
          '</p>';
      }
      return;
    }
  ...
  async function hardRevertScheduleRevision(revisionId) {
    ...
    if (
      !window.confirm(
        gmT('schedule.hardRevertConfirm') ||
          'Hard revert the live schedule to this version for all managers?'
      )
    ) {
      return;
    }
    ...
    if (fetched.error || !fetched.data) {
      showScheduleNotice(gmT('schedule.historyFailed') || 'Could not load revision.', false);
      return;
    }
    pushScheduleUndoSnapshot();
    try {
      localStorage.setItem(
        SCHEDULE_ASSIGN_KEY,
        JSON.stringify(fetched.data.schedule_assignments || {})
      );
      setScheduleAssignmentsConfirmedJson(JSON.stringify(fetched.data.schedule_assignments || {}));
    } catch (_a) {
      /* ignore */
    }
    scheduleAssignmentsDirty = true;
    var draftPayload = draftSchedulePayloadFromRemote(fetched.data.draft_schedule);
    if (draftPayload && draftPayload.byWeek) {
      draftScheduleByWeekStore = cloneDraftSchedule(draftPayload.byWeek);
      slotOrderByWeekStore = sanitizeSlotOrderByWeek(draftPayload.slotOrderByWeek);
      groupOrderPotentialByWeekStore = sanitizeGroupOrderPotentialByWeek(
        draftPayload.groupOrderPotentialByWeek
      );
      scheduleNetSalesByWeekStore = sanitizeScheduleNetSalesByWeek(
        draftPayload.scheduleNetSalesByWeek
      );
      try {
        localStorage.setItem(DRAFT_SCHEDULE_BY_WEEK_KEY, JSON.stringify(draftScheduleByWeekStore));
        setDraftScheduleConfirmedJson(JSON.stringify(draftPayload));
      } catch (_d) {
        /* ignore */
      }
      persistSlotOrderStores({ skipDirty: true });
    ...
    showScheduleNotice(gmT('schedule.hardRevertDone') || 'Schedule restored from history.', true);
  }
  ...
  ['scheduleHistoryModalClose', 'scheduleHistoryCancel', 'scheduleHistoryModalBackdrop'].forEach(
    function (id) {
      var el = document.getElementById(id);
      if (el) el.addEventListener('click', closeScheduleHistoryModal);
    }
  );
  if (scheduleHistoryList) {
    scheduleHistoryList.addEventListener('click', function (e) {
      var btn = e.target && e.target.closest ? e.target.closest('[data-hard-revert]') : null;
      if (!btn) return;
      void hardRevertScheduleRevision(btn.getAttribute('data-hard-revert'));
    });
  }
  ...
  var scheduleDownloadWeekBtn = document.getElementById('scheduleDownloadWeekBtn');
  if (scheduleDownloadWeekBtn) {
    scheduleDownloadWeekBtn.addEventListener('click', function () {
      void downloadSelectedWeekScheduleExcel();
    });
    updateScheduleDownloadWeekButton();
  }
  ...
  var templateShiftEditCloseBtn = document.getElementById('templateShiftEditClose');
  if (templateShiftEditCloseBtn) {
    templateShiftEditCloseBtn.addEventListener('click', function () {
      closeTemplateShiftEditPanel();
    });
  }
  ...
  if (applyScheduleTemplateBtn) {
    var applyConfirmPanel = document.getElementById('applyScheduleTemplateConfirm');
    var applyConfirmMsg = document.getElementById('applyScheduleTemplateConfirmMsg');
    var applyConfirmCancel = document.getElementById('applyScheduleTemplateConfirmCancel');
    var applyConfirmYes = document.getElementById('applyScheduleTemplateConfirmYes');

    function hideApplyScheduleTemplateConfirm() {
      if (applyConfirmPanel) applyConfirmPanel.hidden = true;
    }

    function runApplyScheduleTemplatePreview() {
      if (!scheduleTemplateEditorState) {
        scheduleTemplateEditorState = currentTemplateEditorState();
      }
      var range = formatScheduleWeekRangeLabel(scheduleCalendarWeekIndex);
      ...
      if (applyConfirmYes) applyConfirmYes.disabled = true;
      hideApplyScheduleTemplateConfirm();
      var applyResult = commitScheduleTemplateEditorToLive();
      var appliedSlots = applyResult && applyResult.appliedSlots ? applyResult.appliedSlots : 0;
      if (!applyResult || !applyResult.ok) {
        if (applyResult && applyResult.reason === 'view_only') {
          showScheduleNotice(gmT('schedule.viewOnlyOtherStoreHint') || 'View only at this location.', false);
        } else {
          showScheduleNotice('Could not apply the preview to this week.', false);
        }
        if (applyBtn) applyBtn.disabled = false;
        if (applyConfirmYes) applyConfirmYes.disabled = false;
        return;
      }
    ...
    applyScheduleTemplateBtn.addEventListener('click', function (ev) {
      if (ev && typeof ev.preventDefault === 'function') ev.preventDefault();
      if (ev && typeof ev.stopPropagation === 'function') ev.stopPropagation();
      var deleteConfirmPanel = document.getElementById('deleteScheduleTemplateConfirm');
      if (deleteConfirmPanel) deleteConfirmPanel.hidden = true;
      window.__gmPendingDeleteTemplateId = '';
      window.__gmPendingDeleteTemplateName = '';
      var range = formatScheduleWeekRangeLabel(scheduleCalendarWeekIndex);
      if (applyConfirmMsg) {
        applyConfirmMsg.textContent =
          'Apply this preview to the live schedule for ' +
          range +
          '? This replaces the current week’s staffing and shift times.';
      }
      if (applyConfirmPanel) {
        applyConfirmPanel.hidden = false;
        if (applyConfirmYes) {
          setTimeout(function () {
            applyConfirmYes.focus();
          }, 0);
        }
        return;
      }
      ...
      runApplyScheduleTemplatePreview();
    });
    if (applyConfirmCancel) {
      applyConfirmCancel.addEventListener('click', function () {
        hideApplyScheduleTemplateConfirm();
      });
    }
    if (applyConfirmYes) {
      applyConfirmYes.addEventListener('click', function () {
        runApplyScheduleTemplatePreview();
      });
    }
  }
  if (deleteScheduleTemplateBtn) {
    deleteScheduleTemplateBtn.addEventListener('click', function (ev) {
      if (ev && typeof ev.preventDefault === 'function') ev.preventDefault();
      if (ev && typeof ev.stopPropagation === 'function') ev.stopPropagation();
      var selTpl = document.getElementById('scheduleTemplateSelect');
      var chosen = selTpl && selTpl.value ? selTpl.value : '';
      if (!chosen) return;
      var tpl = normalScheduleTemplates().find(function (t) {
        return t && t.id === chosen;
      });
      if (!tpl) return;
      var tplName = tpl.name || 'Untitled';
      var applyConfirmPanel = document.getElementById('applyScheduleTemplateConfirm');
      var deleteConfirmPanel = document.getElementById('deleteScheduleTemplateConfirm');
      var deleteConfirmMsg = document.getElementById('deleteScheduleTemplateConfirmMsg');
      if (applyConfirmPanel) applyConfirmPanel.hidden = true;
      if (deleteConfirmMsg) {
        deleteConfirmMsg.textContent =
          'Delete saved template "' + tplName + '"? This cannot be undone.';
      }
      if (deleteConfirmPanel) {
        window.__gmPendingDeleteTemplateId = chosen;
        window.__gmPendingDeleteTemplateName = tplName;
        deleteConfirmPanel.hidden = false;
        var deleteConfirmYes = document.getElementById('deleteScheduleTemplateConfirmYes');
        if (deleteConfirmYes) {
          setTimeout(function () {
      ...
      flushTeamStateSyncNow();
      populateScheduleTemplateSelect();
      resetScheduleTemplateEditorToCurrentWeek();
      showScheduleNotice('Deleted template "' + tplName + '".', false);
    });
  }
  var deleteConfirmCancelEl = document.getElementById('deleteScheduleTemplateConfirmCancel');
  var deleteConfirmYesEl = document.getElementById('deleteScheduleTemplateConfirmYes');
  if (deleteConfirmCancelEl) {
    deleteConfirmCancelEl.addEventListener('click', function () {
      var panel = document.getElementById('deleteScheduleTemplateConfirm');
      if (panel) panel.hidden = true;
      window.__gmPendingDeleteTemplateId = '';
      window.__gmPendingDeleteTemplateName = '';
    });
  }
  if (deleteConfirmYesEl) {
    deleteConfirmYesEl.addEventListener('click', function () {
      var chosen = window.__gmPendingDeleteTemplateId || '';
      var tplName = window.__gmPendingDeleteTemplateName || 'Untitled';
      var panel = document.getElementById('deleteScheduleTemplateConfirm');
      if (panel) panel.hidden = true;
      window.__gmPendingDeleteTemplateId = '';
      window.__gmPendingDeleteTemplateName = '';
  ...
  if (newMasterScheduleTemplateBtn) {
    newMasterScheduleTemplateBtn.addEventListener('click', function () {
      if (!managerCanEditCurrentRestaurant()) return;
      var masterSel = document.getElementById('masterScheduleTemplateSelect');
  ...
  if (masterScheduleTemplateSelect) {
    masterScheduleTemplateSelect.addEventListener('change', function () {
      ...
      if (chosen) {
        startMasterScheduleTemplateEditor(chosen);
        return;
      }
      scheduleMasterEditorState = null;
  ...
  if (saveMasterScheduleTemplateBtn) {
    saveMasterScheduleTemplateBtn.addEventListener('click', function () {
      var result = saveMasterScheduleTemplateFromEditor();
      if (!result || !result.ok) {
        if (result && result.reason === 'invalid_time') {
          showScheduleNotice('Use HH:MM format for every clock-in and clock-out time.', false);
        } else {
          showScheduleNotice(
            gmT('schedule.masterTemplateNameRequired') ||
              'Enter a Master Template name before saving.',
            false
          );
        }
        return;
  ...
  if (deleteMasterScheduleTemplateBtn) {
    deleteMasterScheduleTemplateBtn.addEventListener('click', function () {
      ...
      showScheduleNotice('Deleted Master Template "' + (chosen.name || 'Untitled') + '".', false);
    });
  }
  ...
  if (empUsualRestaurant) {
    empUsualRestaurant.addEventListener('change', function () {
      syncEmployeePrimaryLocationField();
      syncEmployeeDeliveryTipRetentionField(null, { preferDefault: true });
    });
  }
  ...
  if (empFirstName) {
    empFirstName.addEventListener('change', function () {
      syncEmployeeDeliveryTipRetentionField(null, { preferDefault: true });
    });
  }
  if (empLastName) {
    empLastName.addEventListener('change', function () {
      syncEmployeeDeliveryTipRetentionField(null, { preferDefault: true });
    });
  }

  if (empPrimaryLocation) {
    empPrimaryLocation.addEventListener('change', function () {
      syncEmployeeDeliveryTipRetentionField(null, { preferDefault: true });
    });
  }

  function buildEmployeeFormDraftForTipRetention() {
  ...
  function syncEmployeeDeliveryTipRetentionField(emp, opts) {
    opts = opts || {};
    var isServer =
      emp && emp.staffType
        ? emp.staffType === 'Server'
        : !!(empStaffType && empStaffType.value === 'Server');
    if (empDeliveryTipRetentionWrap) empDeliveryTipRetentionWrap.hidden = !isServer;
    if (!empDeliveryTipRetention) return;
    if (!isServer) {
      empDeliveryTipRetention.value = '';
      return;
    }
    var draft = emp
      ? {
          ...
          usualRestaurant: emp.usualRestaurant,
          meta: emp.meta,
          deliveryTipRetention: emp.deliveryTipRetention,
        }
      : buildEmployeeFormDraftForTipRetention();
    var stored = normalizeDeliveryTipRetention(
    ...
    var def = defaultDeliveryTipRetentionForEmployee(draft);
    if (opts.preferDefault) {
      var cur = normalizeDeliveryTipRetention(empDeliveryTipRetention.value);
      /* Keep a custom value; refresh when empty or still on a standard store default. */
      if (cur == null || cur === 0.8 || cur === 0.95) {
        empDeliveryTipRetention.value = def != null ? String(def) : '';
      }
      return;
    }
    if (stored != null) {
      empDeliveryTipRetention.value = String(stored);
    } else if (def != null) {
      empDeliveryTipRetention.value = String(def);
    } else {
      empDeliveryTipRetention.value = '';
    }
  }
    ...
    deleteEmployeeBtn.addEventListener('click', function () {
      ...
      var ok = window.confirm(
        'Delete "' +
          label +
          '" from the team?\n\n' +
          'They will be removed from the roster and schedule (shifts become Unassigned). ' +
          'Timecard tip/leave extras for this person are cleared, and time clock punches are deleted. ' +
          'This cannot be undone.'
      );
      if (!ok) return;
      deleteEmployeeBtn.disabled = true;
      void deleteEmployeeCompletely(emp).then(function (res) {
        deleteEmployeeBtn.disabled = false;
        if (!res || !res.ok) {
          window.alert((res && res.message) || 'Could not delete employee.');
          return;
        }
        editingEmployeeId = null;
        showScreen(5);
  ...
  if (employeeForm) {
    employeeForm.addEventListener('submit', async function (ev) {
      ...
      if (stSave === 'Server') {
        var dtrRaw = empDeliveryTipRetention
          ? String(empDeliveryTipRetention.value || '').trim()
          : '';
        var dtrNum = dtrRaw === '' ? null : normalizeDeliveryTipRetention(dtrRaw);
        if (dtrNum == null) {
          dtrNum = defaultDeliveryTipRetentionForEmployee({
            firstName: first,
            lastName: last,
            displayName: (first + ' ' + last).trim(),
            staffType: stSave,
      ...
      if (rec.staffType === 'Server' && rec.deliveryTipRetention != null) {
        rec.meta = rec.meta && typeof rec.meta === 'object' ? rec.meta : {};
        rec.meta.deliveryTipRetention = rec.deliveryTipRetention;
      } else if (rec.meta && rec.meta.deliveryTipRetention != null) {
        delete rec.meta.deliveryTipRetention;
        if (rec.deliveryTipRetention != null) delete rec.deliveryTipRetention;
      }
  ...
  function gmCalloutSetLoginGateOpen(isOpen) {
    ...
    if (isOpen) {
      ...
      try {
        loginEl.style.removeProperty('pointer-events');
      } catch (_pe) {
        /* ignore */
      }
    } else {
      /* Avoid closing the gate if authed was cleared mid-restore. */
      if (!document.documentElement.classList.contains('authed')) {
        return;
      }
      loginEl.hidden = true;
  ...
  async function gmCalloutRestoreAuthedShellFromSupabase() {
    ...
    /* Re-check: a concurrent SIGNED_OUT / hard sign-out may have cleared authed. */
    if (!root.classList.contains('authed')) {
      gmCalloutSetLoginGateOpen(true);
      return false;
    }
  ...
  window.gmCalloutFlushTipPayrollSyncNow = flushTipPayrollPushToSupabase;
  ...
  function initGmCalloutTimecardsModule() {
    ...
    window.gmCalloutTimecards.init({
      ...
      tipTakehomePctForDishwasherEmployee: tipTakehomePctForDishwasherEmployee,
      deliveryTipRetentionFactorForEmployee: deliveryTipRetentionFactorForEmployee,
      defaultDeliveryTipRetentionForEmployee: defaultDeliveryTipRetentionForEmployee,
  ...
  window.gmCalloutOnLocaleChange = function (_locale) {
    ...
    if (document.documentElement.classList.contains('manager-app')) {
      ...
      if (screen === 1) {
        ...
        if (typeof updateScheduleDownloadWeekButton === 'function') updateScheduleDownloadWeekButton();
